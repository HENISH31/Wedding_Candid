...
function buildItems(pool, seg) {
  ...
  const oddYs = [-15, -13, -11, -9, -7, -5, -3, -1, 1, 3, 5, 7, 9, 11, 13, 15, 17];
  ...
  useEffect(() => {
    ...
    const drift = () => {
      if (!draggingRef.current && !inertiaRAF.current && !openingRef.current) {
        ...
        const nextY = wrapAngleSigned(rotationRef.current.y + 1.5 * dt);
        
        // Oscillating vertical drift to keep it dynamic but bounded
        const verticalLimit = maxVerticalRotationDeg / 2;
        const driftX = 2 * Math.sin(now / 4000); 
        const nextX = clamp(rotationRef.current.x * 0.99 + driftX * 0.01, -verticalLimit, verticalLimit);
    ...
    (vx, vy) => {
      ...
      const maxFrames = Math.round(90 + 270 * d);
        ...
        const step = () => {
          ...
          const nextX = clamp(rotationRef.current.x - vY / 200, -verticalLimit, verticalLimit);
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        const verticalLimit = maxVerticalRotationDeg / 2;
        const nextX = clamp(startRotRef.current.x - dyTotal / dragSensitivity, -verticalLimit, verticalLimit);
  ...
  const openItemFromElement = el => {
    ...
    const onFirstEnd = ev => {
      ...
      if (aspect > maxWidth / maxHeight) {
        ...
        // Portrait limited by height
        targetHeight = maxHeight;
        targetWidth = maxHeight * aspect;
      }

      const tempWidth = `${targetWidth}px`;
      const tempHeight = `${targetHeight}px`;
